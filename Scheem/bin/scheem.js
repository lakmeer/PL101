...
(function() {
  var DEBUG, defEnv, e, getEnv, lookup, new_binding, store, update;
  ...
  lookup = function(env, name) {
    ...
    for (v in env.vars) {
      if (v === name) return env.vars[name];
    }
  ...
  update = function(env, name, value) {
    ...
    for (v in env.vars) {
      if (v === name) {
        _results.push(env.vars[name] = value);
      } else {
  ...
  e = function(expr, env) {
    ...
    switch (expr[0]) {
      ...
      case 'set!':
        ...
        return expr[2];
      case '+':
        return e(expr[1], env) + e(expr[2], env);
      case '-':
        return e(expr[1], env) - e(expr[2], env);
      case '*':
        return e(expr[1], env) * e(expr[2], env);
      ...
      case 'quote':
        return expr[1];
      case 'cons':
        return [e(expr[1], env)].concat(e(expr[2], env));
      case 'car':
        return e(expr[1])[0];
      case 'cdr':
        list = e(expr[1], env);
...
(function() {
  var $editor, cm_editor, cm_output, e, evaluate, examples, p;
  ...
  evaluate = function(source) {
    ...
    try {
      ...
      return cm_output.setValue("Result:\n\n" + String(val));
    } catch (ex) {
      $('#output-area').addClass('error');
      if (ex.name === 'SyntaxError') {
        return cm_output.setValue("Syntax error:\n\n" + String(ex).replace("SyntaxError: ", "Expected one of:\n  ").replace(/\,/g, '\n  '));
      } else {
        return cm_output.setValue("Scheem error:\n\n" + String(ex));
      }
    }
  };
